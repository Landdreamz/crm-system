...
import { parseMLSFromHARText, parseAddressFromHARText } from './landCalculatorParseMLS';

/** Remove trailing " Word County" (e.g. "78616 Bastrop County" -> "78616") so full address is street, city, state, zip only */
...
export const LandCalculator: React.FC = () => {
  ...
  const [parseAddressResult, setParseAddressResult] = useState<string>('');
  ...
  const [clipboardTemplate, setClipboardTemplate] = useState<string>('');
  ...
  const buildCopySummary = (): string => {
    ...
    const pct = (p: number) => estimatedMarketValue ? estimatedMarketValue * (p / 100) : 0;
    const copySummaryPercentBands = [90, 80, 70, 65, 60, 50, 40, 30, 20, 10] as const;
    const percentLines = copySummaryPercentBands
      .map((p) => `• ${p}% = $ ${fmtMoneyNum(pct(p))}`)
      .join('\n');
    ...
    const activeListingPriceLine = (activeListingData.listPrice && activeListingData.daysOnMarket)
      ? `${fmtMoney(activeListingData.listPrice)} (${formatDateMMDDYYYY(activeListingData.listDate) || '___'} / ${activeListingData.daysOnMarket} DOM)`
      : '___________________';

    const fullSummary =
      `COMPARABLES\n` +
      `COMPS BY: ___________________\n` +
      `Date: ${today}\n` +
      `Active Listing Price (DATE / DOM): $ ${activeListingPriceLine}\n` +
      `Listing Agent: ___________________\n` +
      `Listing Agent Phone: ___________________\n` +
      `Expired Listing Price (DATE / DOM): $ ___________________\n` +
      `Previous Purchase Price: $ ___________________\n` +
      `Builder Sold Comps: ☐ Yes ☐ No\n` +
      `Active Listings Over 90 Days:\n\n` +
      `Subject Property: ${placeholder(address)}\n` +
      `Asking Price: ${placeholder(baseValue ? fmtMoney(baseValue) : '')}\n` +
      `Acres / Sqft: ${subjectLotSize || '___________________'}\n` +
      `Tree Density: ${placeholder(trees)}\n` +
      `Flood Zone: ${placeholder(floodZone)}\n\n` +
  ...
  // Seed clipboard template once on mount so the box shows current template (user can edit and regenerate anytime)
  useEffect(() => {
    setClipboardTemplate(buildCopySummary());
  ...
  const parseInputData = () => {
    // Parse and set address first, so it always runs even if rest of parse fails
    let newAddress = '';
    try {
      newAddress = parseAddressFromHARText(pasteInput);
    } catch (_) {}
    if (newAddress) {
      setAddress(newAddress);
      setParseAddressResult(`Address found: ${newAddress}`);
    } else {
      const preview = pasteInput.length > 0 ? pasteInput.slice(0, 120).replace(/\n/g, ' ') : '(empty)';
      setParseAddressResult(`No address detected. Paste has ${pasteInput.length} characters. Start: "${preview}..."`);
    }
    ...
    try {
      ...
      if (!newAddress) {
        const again = parseAddressFromHARText(pasteInput);
        if (again) newAddress = again;
      }
      ...
      lines.forEach(line => {
        ...
        // Look for address (only if not already set by parseAddressFromHARText)
        if (!newAddress) {
          if (lowerLine.includes('address:') || lowerLine.includes('located at') || lowerLine.includes('property address')) {
            const addressMatch = t.match(/(?:address:|located at|property address:?)\s*(.+)/i);
            if (addressMatch) {
              const raw = addressMatch[1].trim();
              newAddress = raw.includes('\t') ? raw.split('\t')[0].trim() : raw;
            }
          } else if (/^\d+\s+[\w\s]+,\s*[^,]+,\s*[^,]+,\s*\d{5}/.test(t)) {
            newAddress = stripCountySuffix(t);
          } else if (/^\d+\s+\w+/.test(t)) {
            newAddress = stripCountySuffix(t);
          }
        }
        ...
        if (lowerLine.includes('sq ft') || lowerLine.includes('sqft') || lowerLine.includes('square feet') || lowerLine.includes('square foot')) {
          const sqftMatch = t.match(/(\d+[,\d]*\.?\d*)\s*(sq\.?\s*ft\.?|sqft|square\s*feet?)/i);
          if (sqftMatch) {
        ...
        // Look for acreage if square footage wasn't found
          ...
          const acreMatch = t.match(/(\d+\.?\d*)\s*acre/i);
          if (acreMatch) {
        ...
        if (lowerLine.includes('apprais') || lowerLine.includes('county value')) {
          const appraisalMatch = t.match(/\$?\s*([\d,]+)/);
          if (appraisalMatch) {
      ...
      // Fallback: parse address from full paste
      if (!newAddress) newAddress = parseAddressFromHARText(pasteInput);
      // Last-resort: find line that has both "address" and "orig price" or "list price", extract street
      if (!newAddress && pasteInput) {
        const pasteLines = pasteInput.split(/\r\n|\r|\n/);
        for (const line of pasteLines) {
          ...
          if (/office|mopac|78746/i.test(line)) continue;
          const addrPos = lower.indexOf('address');
          const afterAddr = line.slice(addrPos + 7);
          const colonPos = afterAddr.indexOf(':');
          const start = colonPos >= 0 ? colonPos + 1 : 0;
          let street = afterAddr.slice(start).trim();
      ...
      // Update state with parsed values (address + message already set at start; set again if loop found it)
  ...
  const parseComp1InputData = () => {
    try {
      ...
      // Use same HAR/MLS address parser as subject property (handles "106 Passion Vine Road, Dale, Texas, 78616" etc.)
      newData.address = parseAddressFromHARText(comp1PasteInput) || '';
      let foundAddress = !!newData.address;
      ...
      // First pass - look for address if parser didn't find one
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
          ...
          if (mlsMatch && !line.toLowerCase().includes('agent') && !line.toLowerCase().includes('office')) {
            newData.address = stripCountySuffix(line);
            foundAddress = true;
            break;
          }
      ...
      // If still no address, try lines with street suffixes
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
          const line = lines[i].trim();
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!newData.mlsNumber && (lowerLine.includes('ml#') || lowerLine.includes('mls#'))) {
          const mlsMatch = line.match(/ML#:\s*([A-Za-z0-9]+)/i) || line.match(/MLS\s*#?\s*:\s*([A-Za-z0-9]+)/i) || line.match(/ML#\s*([A-Za-z0-9]+)/i);
          if (mlsMatch) {
  ...
  const parseComp2InputData = () => {
    try {
      ...
      const newData = {
        acres: '', squareFeet: '', salePrice: '', appraisalValue: '', address: '', mlsNumber: '', debrisLevel: '', slope: '', trees: '', needsWell: '', needsSeptic: '', floodZone: '' as FloodZoneValues | '', dom: '', pricePerSqFt: '', pricePerAcre: '', closeDate: ''
      };
      newData.address = parseAddressFromHARText(comp2PasteInput) || '';
      let foundAddress = !!newData.address;
      ...
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
          ...
          if (mlsAddressRegex.test(line) && !line.toLowerCase().includes('agent') && !line.toLowerCase().includes('office')) {
            newData.address = stripCountySuffix(line); foundAddress = true; break;
          }
          if (line.includes('Address:') && !line.toLowerCase().includes('office') && !line.toLowerCase().includes('agent')) {
            const addr = extractValue(line);
            newData.address = addr.includes('\t') ? addr.split('\t')[0].trim() : addr; foundAddress = true; break;
          }
        }
      }
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
          ...
          if (/^\d+\s+[^,]+(?:Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Drive|Dr|Way|Court|Ct|Circle|Cir|Boulevard|Blvd|Highway|Hwy)/i.test(line)) {
            newData.address = stripCountySuffix(line); break;
          }
        }
      }
  ...
  const parseComp3InputData = () => {
    try {
      ...
      newData.address = parseAddressFromHARText(comp3PasteInput) || '';
      let foundAddress = !!newData.address;
      const extractValue = (line: string) => { const parts = line.split(':'); return parts.length > 1 ? parts[1].trim() : ''; };
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
  ...
  const parseComp4InputData = () => {
    try {
      ...
      newData.address = parseAddressFromHARText(comp4PasteInput) || '';
      let foundAddress = !!newData.address;
      const extractValue = (line: string) => { const parts = line.split(':'); return parts.length > 1 ? parts[1].trim() : ''; };
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!newData.mlsNumber && (lowerLine.includes('ml#') || lowerLine.includes('mls#'))) { const m = line.match(/ML#:\s*([A-Za-z0-9]+)/i) || line.match(/MLS\s*#?\s*:\s*([A-Za-z0-9]+)/i) || line.match(/ML#\s*([A-Za-z0-9]+)/i); if (m) newData.mlsNumber = m[1]; }
  ...
  const parseComp5InputData = () => {
    try {
      ...
      newData.address = parseAddressFromHARText(comp5PasteInput) || '';
      let foundAddress = !!newData.address;
      const extractValue = (line: string) => { const parts = line.split(':'); return parts.length > 1 ? parts[1].trim() : ''; };
      if (!foundAddress) {
        for (let i = 0; i < Math.min(20, lines.length); i++) {
  ...
  const parseActiveListingInputData = () => {
    try {
      ...
      const newData = {
        ...
        listPrice: '',
        address: '',
        mlsNumber: parseMLSFromHARText(activeListingPasteInput) || '',
        debrisLevel: '',
        slope: '',
        trees: '',
        needsWell: '',
      ...
      for (let i = 0; i < Math.min(20, lines.length); i++) {
        ...
        if (mlsMatch && !line.toLowerCase().includes('agent') && !line.toLowerCase().includes('office')) {
          newData.address = stripCountySuffix(line); // Remove county if present
          foundAddress = true;
          break;
        }
      ...
      if (!foundAddress) newData.address = parseAddressFromHARText(activeListingPasteInput) || '';
  ...
  const parseActiveListing2InputData = () => {
    try {
      ...
      const newData = {
        acres: '', squareFeet: '', listPrice: '', address: '', mlsNumber: parseMLSFromHARText(activeListing2PasteInput) || '', debrisLevel: '', slope: '', trees: '', needsWell: '', needsSeptic: '',
      ...
      if (!foundAddress) newData.address = parseAddressFromHARText(activeListing2PasteInput) || '';
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        const lowerLine = line.toLowerCase();
        if (line.includes('Address:')) continue;
        if (!newData.mlsNumber && (lowerLine.includes('ml#') || lowerLine.includes('mls#'))) {
          const m = line.match(/ML#:\s*([A-Za-z0-9]+)/i) || line.match(/MLS\s*#?\s*:\s*([A-Za-z0-9]+)/i) || line.match(/ML#\s*([A-Za-z0-9]+)/i);
  ...
  return (
    <Card>
      <CardContent>
        ...
        {parseAddressResult && (
          <Alert severity={parseAddressResult.startsWith('Address found') ? 'success' : 'info'} sx={{ mb: 2 }}>
            {parseAddressResult}
          </Alert>
        )}
        ...
        {!showQuickInput && (
          <Button
            ...
            onClick={() => { setShowQuickInput(true); setParseAddressResult(''); }}
            sx={{ mb: 3 }}
          >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
            <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
              <Typography component="span" variant="body2" sx={{ display: 'block', mb: 1.5, color: 'text.secondary', lineHeight: 1.4, whiteSpace: 'nowrap' }}>Slope Condition</Typography>
              <FormControl fullWidth size="small" sx={{ display: 'block' }}>
                <Select
                  value={slope}
                  onChange={(e) => setSlope(e.target.value)}
                  displayEmpty
                  renderValue={(v) => v || 'Select slope condition'}
                >
            ...
            <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
              <Typography component="span" variant="body2" sx={{ display: 'block', mb: 1.5, color: 'text.secondary', lineHeight: 1.4, whiteSpace: 'nowrap' }}>Tree Density</Typography>
              <FormControl fullWidth size="small" sx={{ display: 'block' }}>
                <Select
                  value={trees}
                  onChange={(e) => setTrees(e.target.value)}
                  displayEmpty
                  renderValue={(v) => v || 'Select tree density'}
                >
        ...
        <Box sx={{ mt: 4, pt: 4, borderTop: 1, borderColor: 'divider' }}>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
                <Typography component="label" variant="body2" sx={{ display: 'block', mb: 0.5, color: 'text.secondary' }}>Tree Density</Typography>
                <FormControl fullWidth size="small">
                  <Select
                    value={comp1Data.trees || ''}
                    onChange={(e) => setComp1Data({ ...comp1Data, trees: e.target.value })}
                    displayEmpty
                  >
                    <MenuItem value="">Select tree density</MenuItem>
                    <MenuItem value="none">None</MenuItem>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
                ...
                <FormControl fullWidth size="small">
                  <Select
                    value={comp2Data.trees || ''}
                    onChange={(e) => setComp2Data({ ...comp2Data, trees: e.target.value })}
                    displayEmpty
                  >
                    <MenuItem value="">Select tree density</MenuItem>
                    <MenuItem value="none">None</MenuItem>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
                ...
                <FormControl fullWidth size="small">
                  <Select
                    value={comp3Data.trees || ''}
                    onChange={(e) => setComp3Data({ ...comp3Data, trees: e.target.value })}
                    displayEmpty
                  >
                    <MenuItem value="">Select tree density</MenuItem>
                    <MenuItem value="none">None</MenuItem>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
                <Typography component="label" variant="body2" sx={{ display: 'block', mb: 0.5, color: 'text.secondary' }}>Tree Density</Typography>
                <FormControl fullWidth size="small">
                  <Select value={comp4Data.trees || ''} onChange={(e) => setComp4Data({ ...comp4Data, trees: e.target.value })} displayEmpty>
                    <MenuItem value="">Select tree density</MenuItem>
                    <MenuItem value="none">None</MenuItem>
                    <MenuItem value="moderate">Moderate</MenuItem>
                    <MenuItem value="heavy">Heavy</MenuItem>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              ...
              <Box sx={{ flexGrow: 1, minWidth: { xs: '100%', md: 'calc(33.33% - 8px)' } }}>
                <Typography component="label" variant="body2" sx={{ display: 'block', mb: 0.5, color: 'text.secondary' }}>Tree Density</Typography>
                <FormControl fullWidth size="small">
                  <Select value={comp5Data.trees || ''} onChange={(e) => setComp5Data({ ...comp5Data, trees: e.target.value })} displayEmpty>
                    <MenuItem value="">Select tree density</MenuItem>
                    <MenuItem value="none">None</MenuItem>
                    <MenuItem value="moderate">Moderate</MenuItem>
                    <MenuItem value="heavy">Heavy</MenuItem>
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              <TextField
                ...
                minRows={12}
                fullWidth
                label="Report template (editable)"
                placeholder="Click « Regenerate template » to fill from current data"
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                <Button
                  variant="outlined"
                  onClick={() => setClipboardTemplate(buildCopySummary())}
                >
                  Regenerate template
                </Button>
                <Button
                  variant="contained"
                  onClick={() => {
                    navigator.clipboard.writeText(clipboardTemplate);
                  }}
                >
                  Copy to Clipboard
                </Button>
